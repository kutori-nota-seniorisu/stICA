% 不同的时间成分匹配方法的测试。目前采用的是方法2，另外CBSS的方法也实现了，参数还需要优化。
% 这里使用的数据均为stICA后得到的时间成分
...
%% 时间成分匹配2。以识别出的脉冲串直接进行匹配，由于空间匹配的相关系数是负值，因而在提取脉冲串时需要添加负号。
% 时间成分滤波
fsampu = 2000;
...
%% 时间成分匹配3。以识别出的脉冲串直接进行匹配，采用HWM方法，不需要加负号
fsampu = 2000;
scale = fsampu/45;
for i = 1:size(T,2)
...
for i = 1:length(decompo_pulses)
    for j = 1:length(ipulses)
        ...
        % 容差±30ms
        % 采样频率2000Hz，则1ms对应于2个样本点
        valid_elements = diff_values <= 30*2;
        count = sum(valid_elements(:));
        r = count/(length(decompo_pulses{i})+length(ipulses{j})-count);
        if r > 1
            r = 1;
