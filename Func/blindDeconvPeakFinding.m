function [source, PT, CoV, w_new] = blindDeconvPeakFinding(S, exFactor, nMAD, MPD, mode)
...
% Inputs:
%   S - estimated source
%   exFactor - extend factor
%   nMAD - the number of mean absolute distances
%   MPD - the minimum peak distance (sample)
% Outputs:
...
while true
    ...
    % 固定点迭代
    if mode == 1
        w_new = Z * log(cosh(w_old' * Z))' / size(eS, 2) - mean(tanh(w_old' * Z)) * w_old;
        % w_new = Z * tanh(w_old' * Z)' / L - mean(sech(w_old' * Z).^2) * w_old;
    elseif mode == 2
        % w_new = Z * 1/6 * (w_old' * Z).^3' / size(eS, 2) - mean(1/2*(w_old' * Z).^2) * w_old;
        w_new = Z * 1/2 * (w_old' * Z).^2' / size(eS, 2) - mean(w_old' * Z) * w_old;
    end
    % 归一化处理
