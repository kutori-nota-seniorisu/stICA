...
for sub = [16]
    ...
    params.skew_stICA = 0; % 是否使用带偏度的stICA算法，1表示skew-stICA，0表示stICA(无偏)
    params.MultiDistribution = 0; % 带偏度的stICA前提下，1表示1多概率密度分布函数”，0表示“单一概率密度分布函数”
    params.noise = 10; % 概率密度分布函数中峰值对应的横坐标超过params.noise的认为是噪声

    params.k = 100; % 潜在源的数目/主成分的个数
    params.alpha = 1; % 时空ICA的权重参数,alpha=1时表示空间ICA，alpha=0时表示时间ICA
    params.mode = 'st';
    ...
    save(['./Data/experiment/ICdata/R' num2str(sub) '/stICA_C' num2str(params.k) '.mat'], 'S', 'T', 'Ws0', 'Ws', 'Wt', 'U_hat', 'V_hat', 'params', '-v7.3');
